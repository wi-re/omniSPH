...
inline std::tuple<matrix, matrix, matrix> svd2x2(matrix M) {
    Eigen::JacobiSVD<matrix, Eigen::ComputeThinU | Eigen::ComputeThinV> svd(M);
    vec sing = svd.singularValues();
    matrix S;
    S << sing(0), scalar(0.0), scalar(0.0), sing(1);
    return std::make_tuple(svd.matrixU(), S, svd.matrixV());
