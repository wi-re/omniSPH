...
namespace cheb {
	...
	Domain::Domain(vscalar _breakpoints) :breakpoints(_breakpoints) {
	...
	Domain Domain::merged(const Domain& other) const {
		vscalar all_breakpoints = concatenate(breakpoints, other.breakpoints);
		vscalar unique = uniqueElements(all_breakpoints);
	...
	Domain Domain::restricted(const Domain& other) const {
		...
		for (auto b : dom.breakpoints)
			if (lbnd <= b && b <= rbnd)
				newDoms.push_back(b);
		return Domain(convert<vscalar>(newDoms));
	}
	bool Domain::operator==(const Domain& other) const {
		if (size() != other.size())
			return false;
		auto dpdt = std::abs(breakpoints - other.breakpoints);
		auto htol = std::max(HTOL, HTOL * std::abs(breakpoints.max()));
		return all(dpdt <= htol);
	}
	bool Domain::operator!=(const Domain& other) const {
		return !(*this == other);
	}
