...
namespace cheb {
	...
	// vc types are std::valarray<c> [vscalar vlen vcomplex]
	// function types are used as shorthands for funcitons taking scalars or vscalars and returning them [func, vfunc]
	// vbool is a short hand for bool valarrays and vec2 is a shorthand for a pair of scalars.

	using scalar = double;
	using svec = std::vector<scalar>;
	using vscalar = std::valarray<scalar>;

	using len = int32_t;
	using lvec = std::vector<len>;
	using vlen = std::valarray<len>;

	using complex = std::complex<scalar>;
	using cvec = std::vector<complex>;
	using vcomplex = std::valarray<complex>;

	using func = std::function<scalar(scalar)>;
	using vfunc = std::function<vscalar(vscalar)>;

	using vbool = std::valarray<bool>;
	...
	constexpr auto inline eps = std::numeric_limits<scalar>::epsilon();
	// a slightly higher tolerance level required for certain domain functionality
	constexpr auto inline HTOL = 5. * eps;
