...
namespace cheb {
	IntervalFunction::IntervalFunction(vfunc f, Interval interval) : minterval(interval) {
		auto uifunc = [f, interval](vscalar s) {
			return f(interval(s));
		};
	...
	vscalar IntervalFunction::operator()(vscalar x, method how) const {
	...
	vscalar IntervalFunction::eval(vscalar x, method how) const {
		auto y = x;
		if (how == method::clenshaw)
			return clenshaw(y, _coeffs);
		else {
	...
	vscalar IntervalFunction::values() const {
	...
	vscalar IntervalFunction::call_clenshaw(vscalar x) const {
		return this->operator()(x, method::clenshaw);
	}
	vscalar IntervalFunction::call_bary(vscalar x) const {
		return this->operator()(x, method::bary);
	}
	vscalar IntervalFunction::endvalues() const {
		return this->operator()(support());
	}
	Interval IntervalFunction::interval() const { return minterval; }
	vscalar IntervalFunction::support() const { return vscalar{ minterval.a,minterval.b }; }
	vscalar IntervalFunction::coeffs() const { return _coeffs; }
	...
	IntervalFunction IntervalFunction::prolonged(int32_t n) const {
		...
		if ((n - m) < 0)
			...
			auto vk = convert<svec>(ak);
			for (int32_t i = 0; i < (n - m); ++i)
				vk.push_back(0.0);
			return IntervalFunction(convert<vscalar>(vk), interval());
		}
	...
	IntervalFunction IntervalFunction::restricted(Interval subinterval) const {
		...
		return IntervalFunction(
			[*this](vscalar s) {return this->operator()(s); }, size(), subinterval);
	}
	vscalar IntervalFunction::roots(bool polish)const {
		auto roots = rootsunit(_coeffs);
	...
	IntervalFunction IntervalFunction::antiDerivative() const {
		...
		auto	bk = vscalar(0., n + 1);
		auto rk = arange(2, n + 1);
		if (rk.size() > 0)
			generateSlice(bk, 2, n + 1) = .5 * (slice(ak, 1, n) - slice(ak, 3)) / convert<vscalar>(rk);
		bk[1] = ak[0] - .5 * ak[2];
		auto vk = vscalar(1., n);
		for (int32_t i = 1; i < n; i += 2)
			vk[i] = -1;
		bk[0] = (vk * slice(bk, 1)).sum();
	...
	IntervalFunction IntervalFunction::derivative()const {
		...
		auto n = size();
		auto ak = _coeffs;
		auto zk = vscalar(0., n - 1);
		auto wk = convert<vscalar>(2 * arange(1, n));
		auto vk = wk * slice(ak, 1);
		if (zk.size() > 0) {
			zk[n - 2] = vk[n - 2];
		}
		if (zk.size() > 1) {
	...
	scalar IntervalFunction::defIntegral()const {
		...
		auto ii = concatenate(vscalar{ 2,0 }, 2. / convert<vscalar>(1 - kk * kk));
		auto sum = (ak * ii).sum();
	...
	bool IntervalFunction::operator==(const IntervalFunction& rhs) {
		return (minterval == rhs.minterval) && all(_coeffs == rhs._coeffs);
	}
	...
	IntervalFunction pow(const scalar& lhs, const IntervalFunction& rhs) {
		return IntervalFunction(
			[lhs, rhs](vscalar x) { return std::pow(lhs, rhs(x)); }
		, rhs.interval());
	}
}
