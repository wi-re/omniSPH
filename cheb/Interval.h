...
namespace cheb {
	...
	with scalar value and valarrays.
	...
	*/
	struct Interval {
		...
		inline auto mapToInterval(const vscalar& y) const {
			if (a == -1. && b == 1.) return y;
			return evaluate(y, [*this](scalar y) {return std::fma(y, (.5 * (b - a)), .5 * (a + b)); });
			//return .5 * b * (y + 1.) + .5 * a * (1. - y);
			return y * (.5 * (b - a)) + .5 * (a + b);
		}
		...
		inline auto mapFromInterval(const vscalar& x) const {
			if (a == -1. && b == 1.) return x;
			return evaluate(x, [*this](scalar x){return std::fma(x, (2. / (b - a)), -(a + b) / (b - a)); });
			//return (2. * x - a - b) / (b - a);
			return x * (2. / (b - a)) - (a + b) / (b - a);
		}
		...
		vbool isinterior(const vscalar& x) const;
	};
