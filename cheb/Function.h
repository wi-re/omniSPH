...
namespace cheb {
	...
	*/
	...
	public:
		...
		std::pair<vscalar, vscalar> breakdata;
		...
		Function(vfunc f, std::variant<vlen, len> n, Domain d = Domain{ {-1.,1.} });
		Function(vfunc f, Domain d = Domain{ {-1.,1.} });
		// Arbitrary function constructor with adaptive polynomial degree
		Function(func fd, std::variant<vlen, len> n, Domain d = Domain{ {-1.,1.} });
		...
		vscalar breakpoints()  const; // Return the breakpoints between individual IFs
...
#define chebBin(op)\
	...
	friend Function (::operator op)(const scalar& lhs, const Function& rhs);\
	friend Function (::operator op)(const Function& lhs, const scalar& rhs);
		...
		friend Function pow(const scalar& lhs, const Function& rhs);
		friend Function pow(const Function& lhs, const scalar& rhs);
