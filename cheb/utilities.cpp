...
namespace cheb {
	vscalar internal::merge_duplicates(const vscalar& arr, const vscalar& tols) {
		svec out;
		for (int32_t i = 0; i < arr.size() - 1; ++i)
			if (abs(arr[i] - arr[i + 1]) > tols[i])
				out.push_back(arr[i]);
		out.push_back(*(std::end(arr) - 1));
		return convert<vscalar>(out);
	}
	vscalar internal::merge_duplicates(const vscalar& arr, scalar tols) {
		return merge_duplicates(arr, vscalar(tols, arr.size()));
	}
	...
	std::pair<vscalar, vscalar> compute_breakdata(const std::vector<IntervalFunction>& funs) {
		if (funs.size() == 0)
			return std::pair<vscalar, vscalar>{};
		svec points, values;
		...
		*std::rbegin(y) = yr;
		...
		return std::make_pair(convert<vscalar>(x), convert<vscalar>(y));
	}
}