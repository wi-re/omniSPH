...
namespace cheb {
	...
	*/
	...
	public:
		...
		IntervalFunction() :_coeffs(vscalar{}), minterval(-1, 1) {};
		// Direct constructor expecting a set of polynomial coefficients 
		IntervalFunction(vscalar coeffs, Interval _interval = Interval{ -1.,1. }) :_coeffs(coeffs), minterval(_interval) {}
		...
		IntervalFunction(scalar c, Interval interval = Interval{ -1., 1. }) : IntervalFunction(vscalar{ c }, interval) {}
		...
		// The operator() variant can be called with either scalars or vscalars, eval expects a vscalar.
		scalar operator()(scalar x, method how = defaultEvalScheme) const;
		vscalar operator()(vscalar x, method how = defaultEvalScheme) const;
		vscalar eval(vscalar x, method how = defaultEvalScheme) const;
		vscalar call_clenshaw(vscalar x) const;
		vscalar call_bary(vscalar x) const;

		// Property functions
		Interval interval() const; // Returns the interval over which this function is defined
		vscalar endvalues() const; // Returns the values of the function at the interval ends
		vscalar support() const; // Returns the interval as a vscalar
		scalar vscale() const; // Returns the vertical magnitude of the function

		// coefficient functions
		vscalar coeffs() const; // Returns the underlying polynomial coefficients
		vscalar values() const; // Returns the function values at the polynomial nodes
		...
		vscalar roots(bool polish = true)const;
...
friend IntervalFunction (::operator op)(const IntervalFunction& lhs, const scalar& rhs);\
friend IntervalFunction (::operator op)(const scalar& lhs, const IntervalFunction& rhs);
		...
		friend IntervalFunction pow(const IntervalFunction& lhs, const scalar& rhs);
		friend IntervalFunction pow(const scalar& lhs, const IntervalFunction& rhs);
