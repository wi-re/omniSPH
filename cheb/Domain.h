...
namespace cheb {
	...
	*/
	class Domain {
	public:
		const vscalar breakpoints;
		// A domain can either be constructed from a vscalar describing the actual
		...
		Domain(vscalar _breakpoints = { -1.,1. });
		explicit Domain(Interval interval) :Domain(vscalar{ interval.a, interval.b }) {}
		Domain(std::initializer_list<scalar> list) :Domain(vscalar{ list }) {}
