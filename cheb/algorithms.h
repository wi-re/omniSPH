...
namespace cheb {
	...
	vscalar fft(vscalar vals);
	// Inverse fft, see fft above for a description
	vscalar ifft(vscalar vals);
	...
	vscalar rootsunit(vscalar ak, scalar htol = 1e2 * eps);
	...
	vscalar newtonroots(cheb::IntervalFunction fun, vscalar rts, scalar tol = 2.0 * eps, uint32_t maxiter = 10);
	...
	inline scalar bary(scalar xx, vscalar fk, vscalar xk, vscalar vk) {
		return bary(vscalar{ xx }, fk, xk, vk)[0];
	}
	...
	inline scalar clenshaw(scalar xx, vscalar ak) {
		return clenshaw(vscalar{ xx }, ak)[0];
	}
	...
	len standard_chop(vscalar coeffs, scalar tol = 1e2 * eps);
	...
	vscalar adaptive(vfunc fun, uint32_t maxpow2 = 12);
